...
format long g
%%
filename = 'mizu3260.16o';
file = fopen(filename);%mizu3260.16o  mizu0770.15o
sat = {};
while true
...
close all 
name = "G09";
index = find(strcmpi(char(sat(:,1)),name) ); %G05 G02 G03 G06 G09 G14 G17 G19 G22 G23 G24
name = append(name,filename(1:4));
f1 = 1575.42; 
f2 = 1227.6;
...
hold on
plot(t(j),li(j),'o','markersize',5,'markerfacecolor','r')
xlabel('Time_(_s_)')
ylabel('L_I_(_m_)')
title(append('L_2-L_1',name))
legend('L_I = L_2-L_1','cycle-slip point')
...
BW = LW-PN;
figure
plot(BW,'.','markersize',3)
hold on
plot(cycle_slip_BW(1),BW(cycle_slip_BW(1)),'o','markersize',5,'markerfacecolor','r')
xlabel('epoch')
ylabel('B_W_(_m_)')
title(append('MW',name))
legend('BW','cycle-slip point')
